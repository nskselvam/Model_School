...
const valuation_Barcode_Fetch = asyncHandler(async (req, res) => {
  ...
  const ValuationSession = "Y";
  const ValuationSessionTime = "14:30";
  ...
  const barcodeCount = await model.count({
    where: {
      Evaluator_Id: Eva_Id,
      subcode: subcode,
  ...
  if (parseInt(barcodeCount) >= parseInt(Max_Papers)) {
    res.status(201);
  ...
  if (
    ...
    parseInt(barcodeCount) >= parseInt(Sub_Max_Papers)
  ) {
    res.status(201);
    throw new AppError(
  ...
  if (ValuationSession == "Y") {
    if (
      getCurrentISTDateTime().split(" ")[1] >= ValuationSessionTime &&
