...
const ValuationMain = () => {
  ...
  const batchname = valuationBarcode?.data?.barcode;
  ...
  }, [batchname, valuationdatFromBack, finalSubCode])
  ...
  const START_IMG = 3;
  const END_IMG = Number(valuationBarcode?.data?.ImgCnt); // inclusive (fixed max per requirement)
  ...
  return (
    <>
      <ValuationTemplate
        ...
        rightComponent={<ValuationRight questionMain={data} barcodeData={valuationBarcode} viewedCount={viewedSet.size} totalCount={imageIndices.length} currentPage={displayPage} imgNumber={imgNumber} end_image={END_IMG} responseDataFromValuation={valuationData} onFinalizationComplete={handleFinalizationComplete} />}
      >
